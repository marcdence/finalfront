...
export default {
  ...
  methods: {
    ...
    async deleteItem() {
      try {
        await this.$store.dispatch("departure/deleteDeparture", this.selectedId);
        this.loadItem();
        this.isConfirmation = false
    ...
    async loadItem() {
      await this.$store.dispatch("departure/viewDeparture", {});
    },
    cancel() {},
    async openForm() {
      this.isAddForm = true;
    ...
    async submitHandler() {
      ...
      try {
        if(this.isAddForm) await this.$store.dispatch("departure/addDeparture", this.departure);
        else await this.$store.dispatch("departure/editDeparture", this.departure);
        this.$refs.form.reset();
        this.loadItem();
        this.isAddDeparture = false;
        this.isLoading = false;
